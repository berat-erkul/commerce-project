package com.commercelab.inventoryservice.worker;
...
/**
 * inventory-service'in transactional outbox poller'ı.
 * PENDING satırları batch'le çeker, Kafka'ya basar, status=PUBLISHED yapar.
 * Hata olursa retry_count++ ve PENDING kalır.
 * <p>
 * NOT (Task #12 borç): senkron .get() + scheduled poller → throughput limiti.
 * Async + DLQ + correlation-id propagation prod öncesi şart.
 */
@Slf4j
@Component
